    ...
    return (
        <div>
            ...
            <div className='AdminDash_0'>
                <div style={{ width: side ? '24%' : '0%' }}>
                    <AdminSlider slidevalue={'AdminDash'} />
                </div>

                <div style={{ width: side ? '75%' : '94%', margin: 'auto' }}>
                    <Box display={'flex'} flexDirection={['column', 'column', 'row']} justifyContent={'space-between'} mt={'15px'} mb={'15px'}>
                            ...
                            {/* <Box m={'10px'} display={'flex'} alignItems={'center'}>
                                <Box>
                                    {data && data?.map((ele, index) => (
                                        <Box key={index} display={'flex'} alignItems={'center'} color={'grey'}><Box w={'20px'} h={'14px'} borderRadius={'2px'} mr={'5px'} bg={COLORS[index]}></Box>{ele.name}</Box>
                                    ))}
                                </Box>
                            </Box> */}
                        </Box>
                        <Box w={['100%', '100%', '28%']} display={'flex'}>
                            <Box w={'99%'} p={'7px'} boxShadow={'rgba(0, 0, 0, 0.24) 0px 3px 8px'} display={'flex'} alignItems={'center'}>
                                <Box>
                                {data && data?.map((ele, index) => (
                                    <Box key={index} mt={'5px'} mb={'5px'} display={'flex'} alignItems={'center'} color={'grey'}><Box w={'20px'} h={'14px'} borderRadius={'2px'} mr={'5px'} bg={COLORS[index]}></Box>{ele.name}</Box>
                                ))}
                                </Box>
                            </Box>
                        </Box>
